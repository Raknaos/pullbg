...
| Phase | Statut | Détail |
|---|---|---|
| **0 Banc** | **v0.3** | **68 cas**, score **72,5**. 61 cas **72,5**. Hair **68,8**. Fenêtres **74,7**. `ci.sh` + `worst.json`. |
| **1 Ordres** | **OK** | DeepSeek + `pipeline` honoré (fenetre/timbre/chroma). |
| **2 Moteur** | **int8 + 2 passes + fuse + stick + drop studio** | Cyclorama coloré (coins) retiré du cadre. hair-02 **69,4**. fur-02 encore **60,2**. |
...
- Hair **68,8** (w2-30 ok). fur 67,0 → 68 (fur-02 **60,2**)
- Score 68 cas ≥ 80
