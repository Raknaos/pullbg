...
function fillLobster(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = lobsterVerts(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchLobster(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillLobster(img, 100, 100, 40, 42, 176, 44, 32);
  const holes = punchLobster(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `lobster stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = lobsterVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] > tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "lobster stamp design kept");
  assert(a[my * 200 + mx] > 180, "lobster stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "lobster stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around lobster stamp gone");
  assert(a[60 * 200 + 40] < 16, "lobster stamp between tail fans not filled");
  assert(cut.pipeline === "timbre", "lobster stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillLobster(img, 100, 100, 56, 58, 176, 44, 32);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `lobster product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillLobster(img, 100, 60, 48, 50, 176, 44, 32);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `lobster product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "lobster product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around lobster product gone");
}
...
function fillShrimp(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = shrimpVerts(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchShrimp(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillShrimp(img, 100, 100, 40, 42, 196, 84, 48);
  const holes = punchShrimp(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `shrimp stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = shrimpVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] > tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "shrimp stamp design kept");
  assert(a[my * 200 + mx] > 180, "shrimp stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "shrimp stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around shrimp stamp gone");
  assert(a[48 * 200 + 36] < 16, "shrimp stamp between tail fans not filled");
  assert(cut.pipeline === "timbre", "shrimp stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillShrimp(img, 100, 100, 56, 58, 196, 84, 48);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `shrimp product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillShrimp(img, 100, 60, 48, 50, 196, 84, 48);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `shrimp product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "shrimp product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around shrimp product gone");
}
...
function fillSeahorse(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = seahorseVerts(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchSeahorse(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillSeahorse(img, 100, 100, 40, 42, 48, 140, 132);
  const holes = punchSeahorse(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `seahorse stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = seahorseVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] > tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "seahorse stamp design kept");
  assert(a[my * 200 + mx] > 180, "seahorse stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "seahorse stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around seahorse stamp gone");
  assert(a[126 * 200 + 36] < 16, "seahorse stamp inside tail curl not filled");
  assert(cut.pipeline === "timbre", "seahorse stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillSeahorse(img, 100, 100, 56, 58, 48, 140, 132);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `seahorse product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillSeahorse(img, 100, 60, 48, 50, 48, 140, 132);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `seahorse product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "seahorse product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around seahorse product gone");
}
...
function fillJellyfish(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = jellyfishVerts(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchJellyfish(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillJellyfish(img, 100, 100, 40, 42, 72, 168, 196);
  const holes = punchJellyfish(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `jellyfish stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = jellyfishVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[1] > tip[1]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "jellyfish stamp design kept");
  assert(a[my * 200 + mx] > 180, "jellyfish stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "jellyfish stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around jellyfish stamp gone");
  assert(a[160 * 200 + 120] < 16, "jellyfish stamp between tentacles not filled");
  assert(cut.pipeline === "timbre", "jellyfish stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillJellyfish(img, 100, 100, 56, 58, 72, 168, 196);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `jellyfish product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillJellyfish(img, 100, 60, 48, 50, 72, 168, 196);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `jellyfish product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "jellyfish product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around jellyfish product gone");
}
...
function fillStarfish(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = starfishPoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchStarfish(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillStarfish(img, 100, 100, 40, 42, 214, 112, 64);
  const holes = punchStarfish(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `starfish stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = starfishVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[1] < tip[1]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "starfish stamp design kept");
  assert(a[my * 200 + mx] > 180, "starfish stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "starfish stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around starfish stamp gone");
  assert(a[155 * 200 + 100] < 16, "starfish stamp between arms not filled");
  assert(cut.pipeline === "timbre", "starfish stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillStarfish(img, 100, 100, 56, 58, 214, 112, 64);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `starfish product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillStarfish(img, 100, 60, 48, 50, 214, 112, 64);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `starfish product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "starfish product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around starfish product gone");
}
...
function fillShell(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = shellPoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchShell(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillShell(img, 100, 100, 40, 42, 224, 148, 72);
  const holes = punchShell(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `shell stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = shellVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[1] > tip[1]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "shell stamp design kept");
  assert(a[my * 200 + mx] > 180, "shell stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "shell stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around shell stamp gone");
  assert(a[168 * 200 + 40] < 16, "shell stamp beside ears not filled");
  assert(cut.pipeline === "timbre", "shell stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillShell(img, 100, 100, 56, 58, 224, 148, 72);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `shell product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillShell(img, 100, 60, 48, 50, 224, 148, 72);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `shell product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "shell product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around shell product gone");
}
...
function fillSnail(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = snailPoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchSnail(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillSnail(img, 100, 100, 40, 42, 168, 124, 64);
  const holes = punchSnail(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `snail stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = snailVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[1] < tip[1]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "snail stamp design kept");
  assert(a[my * 200 + mx] > 180, "snail stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "snail stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around snail stamp gone");
  assert(a[32 * 200 + 168] < 16, "snail stamp beside coil not filled");
  assert(cut.pipeline === "timbre", "snail stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillSnail(img, 100, 100, 56, 58, 168, 124, 64);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `snail product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillSnail(img, 100, 60, 48, 50, 168, 124, 64);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `snail product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "snail product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around snail product gone");
}
...
function fillFrog(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = frogPoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchFrog(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillFrog(img, 100, 100, 40, 42, 72, 148, 64);
  const holes = punchFrog(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `frog stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = frogVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[1] < tip[1]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "frog stamp design kept");
  assert(a[my * 200 + mx] > 180, "frog stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "frog stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around frog stamp gone");
  assert(a[100 * 200 + 168] < 16, "frog stamp beside body not filled");
  assert(cut.pipeline === "timbre", "frog stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillFrog(img, 100, 100, 56, 58, 72, 148, 64);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `frog product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillFrog(img, 100, 60, 48, 50, 72, 148, 64);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `frog product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "frog product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around frog product gone");
}
...
function fillLizard(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = lizardPoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchLizard(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillLizard(img, 100, 100, 40, 42, 92, 148, 56);
  const holes = punchLizard(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `lizard stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = lizardVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] < tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "lizard stamp design kept");
  assert(a[my * 200 + mx] > 180, "lizard stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "lizard stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around lizard stamp gone");
  assert(a[36 * 200 + 168] < 16, "lizard stamp beside tail not filled");
  assert(cut.pipeline === "timbre", "lizard stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillLizard(img, 100, 100, 56, 58, 92, 148, 56);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `lizard product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillLizard(img, 100, 60, 48, 50, 92, 148, 56);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `lizard product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "lizard product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around lizard product gone");
}
...
function fillSnake(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = snakePoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchSnake(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillSnake(img, 100, 100, 40, 42, 48, 132, 64);
  const holes = punchSnake(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `snake stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = snakeVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] < tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "snake stamp design kept");
  assert(a[my * 200 + mx] > 180, "snake stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "snake stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around snake stamp gone");
  assert(a[36 * 200 + 168] < 16, "snake stamp beside tail not filled");
  assert(cut.pipeline === "timbre", "snake stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillSnake(img, 100, 100, 56, 58, 48, 132, 64);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `snake product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillSnake(img, 100, 60, 48, 50, 48, 132, 64);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `snake product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "snake product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around snake product gone");
}
...
function fillCrocodile(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = crocodilePoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchCrocodile(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillCrocodile(img, 100, 100, 40, 42, 56, 112, 48);
  const holes = punchCrocodile(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `crocodile stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = crocodileVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] < tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "crocodile stamp design kept");
  assert(a[my * 200 + mx] > 180, "crocodile stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "crocodile stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around crocodile stamp gone");
  assert(a[36 * 200 + 168] < 16, "crocodile stamp beside tail not filled");
  assert(cut.pipeline === "timbre", "crocodile stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillCrocodile(img, 100, 100, 56, 58, 56, 112, 48);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `crocodile product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillCrocodile(img, 100, 60, 48, 50, 56, 112, 48);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `crocodile product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "crocodile product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around crocodile product gone");
}
...
function fillElephant(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = elephantPoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchElephant(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillElephant(img, 100, 100, 40, 42, 108, 108, 112);
  const holes = punchElephant(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `elephant stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = elephantVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] < tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "elephant stamp design kept");
  assert(a[my * 200 + mx] > 180, "elephant stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "elephant stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around elephant stamp gone");
  assert(a[36 * 200 + 168] < 16, "elephant stamp beside tail not filled");
  assert(cut.pipeline === "timbre", "elephant stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillElephant(img, 100, 100, 56, 58, 108, 108, 112);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `elephant product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillElephant(img, 100, 60, 48, 50, 108, 108, 112);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `elephant product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "elephant product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around elephant product gone");
}
...
function fillGiraffe(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = giraffePoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchGiraffe(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillGiraffe(img, 100, 100, 40, 42, 108, 108, 112);
  const holes = punchGiraffe(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `giraffe stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = giraffeVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] < tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "giraffe stamp design kept");
  assert(a[my * 200 + mx] > 180, "giraffe stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "giraffe stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around giraffe stamp gone");
  assert(a[36 * 200 + 168] < 16, "giraffe stamp beside tail not filled");
  assert(cut.pipeline === "timbre", "giraffe stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillGiraffe(img, 100, 100, 56, 58, 108, 108, 112);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `giraffe product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillGiraffe(img, 100, 60, 48, 50, 108, 108, 112);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `giraffe product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "giraffe product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around giraffe product gone");
}
...
function fillZebra(image, cx, cy, rx, ry, rr, gg, bb) {
  const pts = zebraPoly(cx, cy, rx, ry);
  const n = pts.length;
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function punchZebra(img, cx, cy, rx, ry) {
...
{
  const img = rgb(200, 200, 236, 214, 176);
  fillZebra(img, 100, 100, 40, 42, 108, 108, 112);
  const holes = punchZebra(img, 100, 100, 70, 74);
  const guess = classifyImage(img);
  assert(guess.mode === "timbre", `zebra stamp classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  const large = zebraVerts(100, 100, 70, 74);
  let tip = large[0];
  for (const v of large) if (v[0] < tip[0]) tip = v;
  const mx = Math.round(100 + (tip[0] - 100) * 0.72);
  const my = Math.round(100 + (tip[1] - 100) * 0.72);
  assert(a[100 * 200 + 100] > 180, "zebra stamp design kept");
  assert(a[my * 200 + mx] > 180, "zebra stamp paper margin kept (whole piece)");
  assert(a[holes[0][1] * 200 + holes[0][0]] < 16, "zebra stamp perforation punched");
  assert(a[8 * 200 + 8] < 16, "album paper around zebra stamp gone");
  assert(a[36 * 200 + 168] < 16, "zebra stamp beside tail not filled");
  assert(cut.pipeline === "timbre", "zebra stamp uses stamp pipeline");
}

{
  const img = rgb(200, 200, 255, 255, 255);
  fillZebra(img, 100, 100, 56, 58, 108, 108, 112);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `zebra product on white is not a stamp (${guess.kind})`);
}

{
  const img = rgb(200, 120, 8, 8, 8);
  fillZebra(img, 100, 60, 48, 50, 108, 108, 112);
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `zebra product on black is not a stamp (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[60 * 200 + 100] > 180, "zebra product on black kept");
  assert(a[2 * 200 + 2] < 16, "black studio around zebra product gone");
}

{
  ...
  for (let y = 0; y < 120; y++) {
    for (let x = 0; x < 80; x++) {
      const i = (y * 80 + x) * 4;
      d[i] = 60 + ((x * 13 + y * 7) % 90);
      d[i + 1] = 70 + ((x * 5 + y * 11) % 80);
      d[i + 2] = 80 + ((x * 3 + y * 17) % 100);
      d[i + 3] = 255;
    }
  }
  ...
  const guess = classifyImage(img);
  assert(guess.mode !== "timbre", `busy portrait is not a stamp (${guess.kind} / ${guess.reason})`);
  const cut = fastCut(img);
  assert(cut.needsRefine === true, "busy portrait goes to IA, not stamp geo");
}

function officeSmallPaneFacade() {
  const img = rgb(200, 120, 142, 142, 146);
  for (let r = 0; r < 2; r++) {
    for (let c = 0; c < 3; c++) {
      const x = 18 + c * 20;
      const y = 22 + r * 14;
      fillRect(img, x, y, x + 16, y + 10, 18, 22, 28);
    }
  }
  return img;
}
...
{
  const img = officeSmallPaneFacade();
  const guess = classifyImage(img);
  assert(guess.interior && guess.mode === "noir", `office small-pane facade classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[27 * 200 + 26] < 16, "office facade top-left pane punched");
  assert(a[27 * 200 + 66] < 16, "office facade top-right pane punched");
  assert(a[43 * 200 + 46] < 16, "office facade bottom-middle pane punched");
  assert(a[2 * 200 + 2] > 180, "office facade wall kept");
  assert(a[29 * 200 + 36] > 180, "office facade mullion kept");
  assert(cut.pipeline === "écran", "office facade uses screen pipeline");
}

{
  const img = rgb(200, 140, 142, 142, 146);
  fillRect(img, 12, 12, 28, 22, 18, 18, 18);
  fillRect(img, 172, 12, 188, 22, 18, 18, 18);
  fillRect(img, 12, 118, 28, 128, 18, 18, 18);
  fillRect(img, 172, 118, 188, 128, 18, 18, 18);
  const guess = classifyImage(img);
  assert(guess.mode === "ia" || guess.mode === "couleur", `four spaced products on gray stay off window (${guess.kind})`);
  assert(!guess.interior, `four spaced products are not an office grid (${guess.kind})`);
}

{
  ...
  for (let r = 0; r < 2; r++) {
    for (let c = 0; c < 3; c++) {
      const x = 18 + c * 16;
      const y = 22 + r * 11;
      fillRect(img, x, y, x + 12, y + 7, 18, 22, 28);
    }
  }
  const guess = classifyImage(img);
  assert(guess.interior && guess.mode === "noir", `tiny office panes classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[25 * 200 + 24] < 16, "tiny office top-left pane punched");
  assert(a[25 * 200 + 56] < 16, "tiny office top-right pane punched");
  assert(a[36 * 200 + 40] < 16, "tiny office bottom-middle pane punched");
  assert(a[2 * 200 + 2] > 180, "tiny office wall kept");
  assert(a[25 * 200 + 31] > 180, "tiny office mullion kept");
  assert(cut.pipeline === "écran", "tiny office uses screen pipeline");
}
...

{
  ...
  fillRect(img, 18, 64, 30, 74, 18, 22, 28);
  fillRect(img, 50, 64, 62, 74, 18, 22, 28);
  const guess = classifyImage(img);
  assert(guess.interior && guess.mode === "noir", `office U-bay classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[27 * 200 + 24] < 16, "office U-bay top-left pane punched");
  assert(a[27 * 200 + 56] < 16, "office U-bay top-right pane punched");
  assert(a[69 * 200 + 24] < 16, "office U-bay bottom-left pane punched");
  assert(a[69 * 200 + 56] < 16, "office U-bay bottom-right pane punched");
  assert(a[2 * 200 + 2] > 180, "office U-bay wall kept");
  assert(a[27 * 200 + 31] > 180, "office U-bay top mullion kept");
  assert(cut.pipeline === "écran", "office U-bay uses screen pipeline");
}

{
  ...
  fillRect(img, 188, 12, 204, 22, 18, 18, 18);
  fillRect(img, 12, 130, 28, 140, 18, 18, 18);
  fillRect(img, 188, 130, 204, 140, 18, 18, 18);
  const guess = classifyImage(img);
  assert(guess.mode === "ia" || guess.mode === "couleur", `five spaced U products stay off bay (${guess.kind})`);
  assert(!guess.interior, `five spaced U products are not an office bay (${guess.kind})`);
}

{
  ...
  fillRect(img, 34, 64, 46, 74, 18, 22, 28);
  const guess = classifyImage(img);
  assert(guess.interior && guess.mode === "noir", `office T-transom classified (${guess.kind})`);
  const cut = fastCut(img);
  const a = alphaOf(cut.image);
  assert(a[27 * 200 + 24] < 16, "office T-transom left pane punched");
  assert(a[27 * 200 + 56] < 16, "office T-transom right pane punched");
  assert(a[69 * 200 + 40] < 16, "office T-transom stem punched");
  assert(a[2 * 200 + 2] > 180, "office T-transom wall kept");
  assert(a[27 * 200 + 31] > 180, "office T-transom mullion kept");
  assert(cut.pipeline === "écran", "office T-transom uses screen pipeline");
}

{
  ...
  fillRect(img, 100, 12, 116, 22, 18, 18, 18);
  fillRect(img, 188, 12, 204, 22, 18, 18, 18);
  fillRect(img, 100, 130, 116, 140, 18, 18, 18);
  const guess = classifyImage(img);
  assert(guess.mode === "ia" || guess.mode === "couleur", `four spaced T products stay off tee (${guess.kind})`);
  assert(!guess.interior, `four spaced T products are not an office tee (${guess.kind})`);
}
