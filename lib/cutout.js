...
export function punchInterior(image) {
  const { data, width: w, height: h } = image;
  const lum = lumaMap(data, w, h);
  ...
  const use = areaB >= areaW ? punchB : punchW;
...
function detectPrintedHoles(lum, w, h, scale = 1) {
  const strip = Math.max(10, Math.round(16 * scale));
...
export function stampCut(image) {
  ...
  const thr = darkBackdrop ? adaptiveDarkThreshold(lum, w, h) : 36;
  const cand = new Uint8Array(w * h);
  ...
  punchHoles(kept, w, h, detectPrintedHoles(lum, w, h, 1));
