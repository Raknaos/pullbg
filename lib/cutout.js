...
export function stampCut(image) {
  ...
  for (const part of parts) {
    ...
    for (const hole of detectPrintedHoles(lum, w, h, localScale, search)) {
      if (holeNearBox(hole, part, pad)) holes.push(hole);
    }
  }
  punchHoles(fg, lum, w, h, holes);
  const body = largestForeground(fg, w, h, 1);
  let alpha = body;
  const paintRect = (rect, localHoles) => {
  ...
  if (parts.length < 2) paintRect(stampRectFromHoles(edgeHoles, w, h), edgeHoles);
  for (const part of parts) {
    const pad = holePad(part);
    const local = [];
    for (const hole of holes) if (holeNearBox(hole, part, pad)) local.push(hole);
    paintRect(stampRectFromHoles(local, w, h), local);
  }
