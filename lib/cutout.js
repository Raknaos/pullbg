  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      while (head < tail) {
        ...
        if (cx > 0) {
          const ni = cy * w + (cx - 1);
          if (mask[ni] && !seen[ni]) { seen[ni] = 1; qx[tail] = cx - 1; qy[tail] = cy; tail++; }
        }
        if (cx + 1 < w) {
          const ni = cy * w + (cx + 1);
          if (mask[ni] && !seen[ni]) { seen[ni] = 1; qx[tail] = cx + 1; qy[tail] = cy; tail++; }
        ...
        if (cy > 0) {
          const ni = (cy - 1) * w + cx;
          if (mask[ni] && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy - 1; tail++; }
        }
        if (cy + 1 < h) {
          const ni = (cy + 1) * w + cx;
          if (mask[ni] && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy + 1; tail++; }
      ...
      for (let i = 0; i < tail; i++) keep[qy[i] * w + qx[i]] = 1;
    }
  }
  return keep;
}
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      while (head < tail) {
        ...
        if (cx > 0) {
          const ni = cy * w + (cx - 1);
          if (alpha[ni] >= minA && !seen[ni]) { seen[ni] = 1; qx[tail] = cx - 1; qy[tail] = cy; tail++; }
        }
        if (cx + 1 < w) {
          const ni = cy * w + (cx + 1);
          if (alpha[ni] >= minA && !seen[ni]) { seen[ni] = 1; qx[tail] = cx + 1; qy[tail] = cy; tail++; }
        ...
        if (cy > 0) {
          const ni = (cy - 1) * w + cx;
          if (alpha[ni] >= minA && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy - 1; tail++; }
        }
        if (cy + 1 < h) {
          const ni = (cy + 1) * w + cx;
          if (alpha[ni] >= minA && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy + 1; tail++; }
      ...
      const idx = new Int32Array(tail);
      for (let i = 0; i < tail; i++) idx[i] = qy[i] * w + qx[i];
      parts.push({ cells: idx, interior: !border });
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      while (head < tail) {
        ...
        if (cx > 0) {
          const ni = cy * w + (cx - 1);
          if (alpha[ni] >= 20 && !seen[ni]) { seen[ni] = 1; qx[tail] = cx - 1; qy[tail] = cy; tail++; }
        }
        if (cx + 1 < w) {
          const ni = cy * w + (cx + 1);
          if (alpha[ni] >= 20 && !seen[ni]) { seen[ni] = 1; qx[tail] = cx + 1; qy[tail] = cy; tail++; }
        ...
        if (cy > 0) {
          const ni = (cy - 1) * w + cx;
          if (alpha[ni] >= 20 && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy - 1; tail++; }
        }
        if (cy + 1 < h) {
          const ni = (cy + 1) * w + cx;
          if (alpha[ni] >= 20 && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy + 1; tail++; }
  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      while (head < tail) {
        ...
        if (cx > 0) {
          const ni = cy * w + (cx - 1);
          if (interior[ni] && !seen[ni]) { seen[ni] = 1; qx[tail] = cx - 1; qy[tail] = cy; tail++; }
        }
        if (cx + 1 < w) {
          const ni = cy * w + (cx + 1);
          if (interior[ni] && !seen[ni]) { seen[ni] = 1; qx[tail] = cx + 1; qy[tail] = cy; tail++; }
        ...
        if (cy > 0) {
          const ni = (cy - 1) * w + cx;
          if (interior[ni] && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy - 1; tail++; }
        }
        if (cy + 1 < h) {
          const ni = (cy + 1) * w + cx;
          if (interior[ni] && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy + 1; tail++; }
      ...
      if (tail <= maxArea) {
        for (let i = 0; i < tail; i++) out[qy[i] * w + qx[i]] = 255;
      }
    }
  }
