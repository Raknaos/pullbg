...
export function stampCut(image) {
  ...
  const holes = edgeHoles.concat(ringHoles, hexHoles, octHoles, pentHoles, triHoles, starHoles, heartHoles);
  ...
  const paintPiece = (localHoles, hexLocal, octLocal, pentLocal, triLocal, starLocal, heartLocal, self) => {
  ...
  if (parts.length < 2) paintPiece(edgeHoles.concat(ringHoles), edgeHoles.concat(hexHoles), octHoles, pentHoles, triHoles, starHoles, heartHoles, null);
  for (let p = 0; p < parts.length; p++) {
    paintPiece(partHoles[p], partHexHoles[p], partOctHoles[p], partPentHoles[p], partTriHoles[p], partStarHoles[p], partHeartHoles[p], parts[p]);
  }
