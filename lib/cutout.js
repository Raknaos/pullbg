...
export function stampCut(image) {
  ...
  const holes = edgeHoles.concat(ringHoles, hexHoles, octHoles, pentHoles, triHoles);
  ...
  const paintPiece = (localHoles, hexLocal, octLocal, pentLocal, triLocal, self) => {
  ...
  if (parts.length < 2) paintPiece(edgeHoles.concat(ringHoles), edgeHoles.concat(hexHoles), octHoles, pentHoles, triHoles, null);
  for (let p = 0; p < parts.length; p++) {
    paintPiece(partHoles[p], partHexHoles[p], partOctHoles[p], partPentHoles[p], partTriHoles[p], parts[p]);
  }
