...
export function punchInterior(image, prefer) {
  ...
  const punchB = labelInteriorLarge(black, w, h, 0.004, 0.72);
  const punchW = labelInteriorLarge(white, w, h, 0.004, 0.72);
  ...
  let use;
  if (prefer === "black" && (areaB || areaM)) {
    if (areaB && areaM) {
      for (let i = 0; i < punchB.length; i++) if (punchM[i]) punchB[i] = 1;
      use = punchB;
    } else {
      use = areaB ? punchB : punchM;
    }
  } else if (prefer === "white" && areaW) {
    use = punchW;
  } else {
    use = areaB >= areaW && areaB >= areaM ? punchB : areaW >= areaM ? punchW : punchM;
  }
