...
export function stampCut(image) {
  ...
  const shaped = ringHoles.length >= 12 || hexHoles.length >= 12 || octHoles.length >= 12
    ...
    || cowHoles.length >= 12 || sheepHoles.length >= 12 || goatHoles.length >= 12 || roosterHoles.length >= 12 || duckHoles.length >= 12;
  if (kept < w * h * 0.08) {
    let borderFg = false;
