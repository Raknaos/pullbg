  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      if (minFill > 0) {
        ...
        if (!bbox || area < bbox * minFill) continue;
      }
...
export function punchInterior(image, prefer) {
  ...
  for (let i = 0; i < lum.length; i++) {
    ...
    else if (Math.abs(lum[i] - borderMean) >= 24) mid[i] = 1;
  }
  const punchB = labelInteriorLarge(black, w, h, 0.004);
  const punchW = labelInteriorLarge(white, w, h, 0.004);
  const punchM = labelInteriorLarge(mid, w, h, 0.004, 0.72);
  let areaB = 0;
  let areaW = 0;
  let areaM = 0;
  ...
  else if (prefer === "black" && areaM) use = punchM;
  else if (prefer === "white" && areaW) use = punchW;
  else use = areaB >= areaW && areaB >= areaM ? punchB : areaW >= areaM ? punchW : punchM;
...
export function stampCut(image) {
  ...
  for (let p = 0; p < parts.length; p++) {
    ...
    for (const hole of detectPrintedHoles(lum, w, h, localScale, search)) {
  ...
  const paintRect = (rect, localHoles, self) => {
    ...
    const restored = keepHoleRect(lum, localHoles, rect, w, h);
    for (let i = 0; i < alpha.length; i++) if (restored[i] > alpha[i]) alpha[i] = restored[i];
  };
  if (parts.length < 2) paintRect(stampRectFromHoles(edgeHoles, w, h), edgeHoles, null);
  for (let p = 0; p < parts.length; p++) {
    paintRect(stampRectFromHoles(partHoles[p], w, h), partHoles[p], parts[p]);
  }
