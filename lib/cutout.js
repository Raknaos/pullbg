...
export function largestForeground(alpha, w, h, minA = 20) {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (tail < floor) continue;
      parts.push({ cells: q.slice(0, tail), interior: !border });
      if (tail > bestArea) bestArea = tail;
    }
  }
...
export function punchInterior(image, prefer) {
  ...
  const punch = (kind) => {
    fillCand(kind);
    const keep = labelInteriorLarge(cand, w, h, 0.004, 0.68, scratch);
    let area = 0;
...
export function stampCut(image) {
  ...
  const sharkHoles = stampDiskFromHoles(ringFit) ? [] : detectSharkHoles(lum, w, h, scale);
  const turtleHoles = stampDiskFromHoles(ringFit) ? [] : detectTurtleHoles(lum, w, h, scale);
  const octopusHoles = stampDiskFromHoles(ringFit) ? [] : detectOctopusHoles(lum, w, h, scale);
  const crabHoles = stampDiskFromHoles(ringFit) ? [] : detectCrabHoles(lum, w, h, scale);
  const lobsterHoles = stampDiskFromHoles(ringFit) ? [] : detectLobsterHoles(lum, w, h, scale);
  const shrimpHoles = stampDiskFromHoles(ringFit) ? [] : detectShrimpHoles(lum, w, h, scale);
  const seahorseHoles = stampDiskFromHoles(ringFit) ? [] : detectSeahorseHoles(lum, w, h, scale);
  const jellyfishHoles = stampDiskFromHoles(ringFit) ? [] : detectJellyfishHoles(lum, w, h, scale);
  const starfishHoles = stampDiskFromHoles(ringFit) ? [] : detectStarfishHoles(lum, w, h, scale);
  const shellHoles = stampDiskFromHoles(ringFit) ? [] : detectShellHoles(lum, w, h, scale);
  const snailHoles = stampDiskFromHoles(ringFit) ? [] : detectSnailHoles(lum, w, h, scale);
  const frogHoles = stampDiskFromHoles(ringFit) ? [] : detectFrogHoles(lum, w, h, scale);
  const lizardHoles = stampDiskFromHoles(ringFit) ? [] : detectLizardHoles(lum, w, h, scale);
  const snakeHoles = stampDiskFromHoles(ringFit) ? [] : detectSnakeHoles(lum, w, h, scale);
  const crocodileHoles = stampDiskFromHoles(ringFit) ? [] : detectCrocodileHoles(lum, w, h, scale);
  const elephantHoles = stampDiskFromHoles(ringFit) ? [] : detectElephantHoles(lum, w, h, scale);
  const giraffeHoles = stampDiskFromHoles(ringFit) ? [] : detectGiraffeHoles(lum, w, h, scale);
  const zebraHoles = stampDiskFromHoles(ringFit) ? [] : detectZebraHoles(lum, w, h, scale);
  ...
  const shaped = ringHoles.length >= 12 || hexHoles.length >= 12 || octHoles.length >= 12
    ...
    || cowHoles.length >= 12 || sheepHoles.length >= 12 || goatHoles.length >= 12 || roosterHoles.length >= 12 || duckHoles.length >= 12 || gooseHoles.length >= 12 || turkeyHoles.length >= 12 || swanHoles.length >= 12 || peacockHoles.length >= 12 || owlHoles.length >= 12 || penguinHoles.length >= 12 || dolphinHoles.length >= 12 || whaleHoles.length >= 12 || sharkHoles.length >= 12 || turtleHoles.length >= 12 || octopusHoles.length >= 12 || crabHoles.length >= 12 || lobsterHoles.length >= 12 || shrimpHoles.length >= 12 || seahorseHoles.length >= 12 || jellyfishHoles.length >= 12 || starfishHoles.length >= 12 || shellHoles.length >= 12 || snailHoles.length >= 12 || frogHoles.length >= 12 || lizardHoles.length >= 12 || snakeHoles.length >= 12 || crocodileHoles.length >= 12 || elephantHoles.length >= 12 || giraffeHoles.length >= 12 || zebraHoles.length >= 12;
  if (kept < w * h * 0.08) {
    let borderFg = false;
  ...
  for (let p = 0; p < parts.length; p++) {
    ...
    const localShark = stampDiskFromHoles(local) ? [] : detectSharkHoles(lum, w, h, localScale, search);
    const localTurtle = stampDiskFromHoles(local) ? [] : detectTurtleHoles(lum, w, h, localScale, search);
    const localOctopus = stampDiskFromHoles(local) ? [] : detectOctopusHoles(lum, w, h, localScale, search);
    const localCrab = stampDiskFromHoles(local) ? [] : detectCrabHoles(lum, w, h, localScale, search);
    const localLobster = stampDiskFromHoles(local) ? [] : detectLobsterHoles(lum, w, h, localScale, search);
    const localShrimp = stampDiskFromHoles(local) ? [] : detectShrimpHoles(lum, w, h, localScale, search);
    const localSeahorse = stampDiskFromHoles(local) ? [] : detectSeahorseHoles(lum, w, h, localScale, search);
    const localJellyfish = stampDiskFromHoles(local) ? [] : detectJellyfishHoles(lum, w, h, localScale, search);
    const localStarfish = stampDiskFromHoles(local) ? [] : detectStarfishHoles(lum, w, h, localScale, search);
    const localShell = stampDiskFromHoles(local) ? [] : detectShellHoles(lum, w, h, localScale, search);
    const localSnail = stampDiskFromHoles(local) ? [] : detectSnailHoles(lum, w, h, localScale, search);
    const localFrog = stampDiskFromHoles(local) ? [] : detectFrogHoles(lum, w, h, localScale, search);
    const localLizard = stampDiskFromHoles(local) ? [] : detectLizardHoles(lum, w, h, localScale, search);
    const localSnake = stampDiskFromHoles(local) ? [] : detectSnakeHoles(lum, w, h, localScale, search);
    const localCrocodile = stampDiskFromHoles(local) ? [] : detectCrocodileHoles(lum, w, h, localScale, search);
    const localElephant = stampDiskFromHoles(local) ? [] : detectElephantHoles(lum, w, h, localScale, search);
    const localGiraffe = stampDiskFromHoles(local) ? [] : detectGiraffeHoles(lum, w, h, localScale, search);
    const localZebra = stampDiskFromHoles(local) ? [] : detectZebraHoles(lum, w, h, localScale, search);
    ...
    for (const hole of localShark) take(hole, partSharkHoles[p]);
    for (const hole of localTurtle) take(hole, partTurtleHoles[p]);
    for (const hole of localOctopus) take(hole, partOctopusHoles[p]);
    for (const hole of localCrab) take(hole, partCrabHoles[p]);
    for (const hole of localLobster) take(hole, partLobsterHoles[p]);
    for (const hole of localShrimp) take(hole, partShrimpHoles[p]);
    for (const hole of localSeahorse) take(hole, partSeahorseHoles[p]);
    for (const hole of localJellyfish) take(hole, partJellyfishHoles[p]);
    for (const hole of localStarfish) take(hole, partStarfishHoles[p]);
    for (const hole of localShell) take(hole, partShellHoles[p]);
    for (const hole of localSnail) take(hole, partSnailHoles[p]);
    for (const hole of localFrog) take(hole, partFrogHoles[p]);
    for (const hole of localLizard) take(hole, partLizardHoles[p]);
    for (const hole of localSnake) take(hole, partSnakeHoles[p]);
    for (const hole of localCrocodile) take(hole, partCrocodileHoles[p]);
    for (const hole of localElephant) take(hole, partElephantHoles[p]);
    for (const hole of localGiraffe) take(hole, partGiraffeHoles[p]);
    for (const hole of localZebra) take(hole, partZebraHoles[p]);
  }
