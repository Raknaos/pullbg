...
export function floodColor(image, maxDist = 32) {
  const { data, width: w, height: h } = image;
  let r = 0;
  let g = 0;
  let b = 0;
  ...
  b /= n;
  const cand = new Uint8Array(w * h);
  for (let p = 0, i = 0; p < cand.length; p++, i += 4) {
    const dist = Math.max(
      Math.abs(data[i] - r),
      Math.abs(data[i + 1] - g),
      Math.abs(data[i + 2] - b),
    );
    cand[p] = dist <= maxDist ? 1 : 0;
  }
  return applyAlpha(image, softAlphaFromBg(connectedToBorder(cand, w, h), w, h));
}
...
export function punchInterior(image, prefer) {
  ...
  const punchT = labelInteriorLarge(tint, w, h, 0.004, 0.72);
  let areaB = 0;
  let areaW = 0;
  let areaM = 0;
  let areaT = 0;
