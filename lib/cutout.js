...
function stampRectFromHoles(holes, w, h) {
  ...
  if (x1 - x0 < w * 0.35 || y1 - y0 < h * 0.35) return null;
  return { x0, y0, x1, y1 };
}
...
export function stampCut(image) {
  ...
  const holes = detectPrintedHoles(lum, w, h, scale);
  punchHoles(fg, lum, w, h, holes);
  const body = largestForeground(fg, w, h, 1);
  const rect = stampRectFromHoles(holes, w, h);
  if (rect && countOpaqueBlobs(body, w, h) < 2 && lightMarginEaten(body, lum, rect, w, h)) {
    return applyAlpha(image, keepHoleRect(lum, holes, rect, w, h));
  }
