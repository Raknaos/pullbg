...
export function stampCut(image) {
  ...
  const holes = edgeHoles.concat(ringHoles, hexHoles);
  ...
  const paintPiece = (localHoles, hexLocal, self) => {
  ...
  if (parts.length < 2) paintPiece(edgeHoles.concat(ringHoles), edgeHoles.concat(hexHoles), null);
  for (let p = 0; p < parts.length; p++) {
    paintPiece(partHoles[p], partHexHoles[p], parts[p]);
  }
