...
export function stampCut(image) {
  ...
  const holes = edgeHoles.concat(ringHoles, hexHoles, octHoles, pentHoles, triHoles, starHoles, heartHoles, crescentHoles, teardropHoles, shieldHoles, crossHoles, arrowHoles, cloudHoles, cloverHoles, flowerHoles, butterflyHoles, leafHoles, fishHoles, birdHoles, catHoles, dogHoles, rabbitHoles, squirrelHoles, foxHoles, bearHoles, horseHoles, pigHoles, cowHoles, sheepHoles, goatHoles, roosterHoles, duckHoles, gooseHoles, turkeyHoles, swanHoles, peacockHoles, owlHoles, penguinHoles, dolphinHoles);
  const shaped = ringHoles.length >= 12 || hexHoles.length >= 12 || octHoles.length >= 12
    ...
    || cowHoles.length >= 12 || sheepHoles.length >= 12 || goatHoles.length >= 12 || roosterHoles.length >= 12 || duckHoles.length >= 12 || gooseHoles.length >= 12 || turkeyHoles.length >= 12 || swanHoles.length >= 12 || peacockHoles.length >= 12 || owlHoles.length >= 12 || penguinHoles.length >= 12 || dolphinHoles.length >= 12;
  if (kept < w * h * 0.08) {
    let borderFg = false;
