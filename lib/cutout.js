...
export function largestForeground(alpha, w, h, minA = 20) {
  ...
  for (const cells of parts) {
    if (cells.length < minKeep) continue;
    for (const i of cells) out[i] = alpha[i];
  }
  return out;
}
...
export function punchInterior(image, prefer) {
  ...
  const paneBlack = Math.max(14, Math.min(22, borderMean - 6));
  ...
  for (let i = 0; i < lum.length; i++) {
    ...
    if (lum[i] >= paneWhite) white[i] = 1;
  }
  const punchB = labelInteriorLarge(black, w, h, 0.012);
  const punchW = labelInteriorLarge(white, w, h, 0.012);
  let areaB = 0;
  let areaW = 0;
...
function countOpaqueBlobs(alpha, w, h) {
  const n = w * h;
  const seen = new Uint8Array(n);
  const qx = new Int32Array(n);
  const qy = new Int32Array(n);
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const start = y * w + x;
      if (alpha[start] < 20 || seen[start]) continue;
      parts++;
      let head = 0;
      let tail = 0;
      qx[tail] = x;
...
export function stampCut(image) {
  ...
  const darkBackdrop = n && borderDark / n >= 0.25 && borderSum / n < 100;
  const cand = new Uint8Array(w * h);
  if (darkBackdrop) {
  ...
  const box = opaqueBox(fg, w, h);
  const inset = box && (box.x0 > 6 || box.y0 > 6 || box.x1 < w - 6 || box.y1 < h - 6);
  let innerHoles = [];
  if (inset) {
    const localScale = Math.max(0.75, Math.min(box.x1 - box.x0, box.y1 - box.y0) / 480);
    innerHoles = detectPrintedHoles(lum, w, h, localScale, box);
  }
  const holes = edgeHoles.concat(innerHoles);
  punchHoles(fg, lum, w, h, holes);
  const body = largestForeground(fg, w, h, 1);
  const edgeRect = stampRectFromHoles(edgeHoles, w, h);
  const innerRect = stampRectFromHoles(innerHoles, w, h);
  const rect = edgeRect || innerRect;
  if (rect && countOpaqueBlobs(body, w, h) < 2 && lightMarginEaten(body, lum, rect, w, h)) {
    return applyAlpha(image, keepHoleRect(lum, edgeRect ? edgeHoles : innerHoles, rect, w, h));
  }
  return applyAlpha(image, body);
}
