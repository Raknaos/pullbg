...
export function stampCut(image) {
  ...
  const shaped = ringHoles.length >= 12 || hexHoles.length >= 12 || octHoles.length >= 12
    ...
    || cowHoles.length >= 12 || sheepHoles.length >= 12 || goatHoles.length >= 12 || roosterHoles.length >= 12 || duckHoles.length >= 12 || gooseHoles.length >= 12;
  if (kept < w * h * 0.08) {
    let borderFg = false;
  ...
  if (parts.length < 2) paintPiece(edgeHoles.concat(ringHoles), edgeHoles.concat(hexHoles), octHoles, pentHoles, triHoles, starHoles, heartHoles, crescentHoles, teardropHoles, shieldHoles, crossHoles, arrowHoles, cloudHoles, cloverHoles, flowerHoles, butterflyHoles, leafHoles, fishHoles, birdHoles, catHoles, dogHoles, rabbitHoles, squirrelHoles, foxHoles, bearHoles, horseHoles, pigHoles, cowHoles, sheepHoles, goatHoles, roosterHoles, duckHoles, gooseHoles, null);
  for (let p = 0; p < parts.length; p++) {
