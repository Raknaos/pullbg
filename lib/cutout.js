...
export function stampCut(image) {
  ...
  const shaped = ringHoles.length >= 12 || hexHoles.length >= 12 || octHoles.length >= 12
    ...
    || bearHoles.length >= 12 || horseHoles.length >= 12;
  if (kept < w * h * 0.08) {
    let borderFg = false;
  ...
  const paintPiece = (localHoles, hexLocal, octLocal, pentLocal, triLocal, starLocal, heartLocal, crescentLocal, teardropLocal, shieldLocal, crossLocal, arrowLocal, cloudLocal, cloverLocal, flowerLocal, butterflyLocal, leafLocal, fishLocal, birdLocal, catLocal, dogLocal, rabbitLocal, squirrelLocal, foxLocal, bearLocal, horseLocal, self) => {
