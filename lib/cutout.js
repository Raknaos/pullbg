...
export function punchInterior(image, prefer) {
  ...
  const punchScratch = { seen: new Uint8Array(w * h), q: new Int32Array(w * h) };
  const punchB = labelInteriorLarge(black, w, h, 0.004, 0.68, punchScratch);
  const punchW = labelInteriorLarge(white, w, h, 0.004, 0.68, punchScratch);
  const punchM = labelInteriorLarge(mid, w, h, 0.004, 0.68, punchScratch);
  const punchT = labelInteriorLarge(tint, w, h, 0.004, 0.68, punchScratch);
  let areaB = 0;
  let areaW = 0;
  let areaM = 0;
  let areaT = 0;
