...
export async function smartCut(file, { mode = "auto", interior = false, onStatus } = {}) {
  ...
  if (useInterior) {
    ...
    push(punched, winner.name + "+intérieur");
    winner = bestOf(cands.filter((c) => c.name.endsWith("intérieur") || c === winner));
    const punchedScore = scoreCut(punched);
    if (punchedScore.score >= winner.score.score * 0.85) {
      winner = { image: punched, name: winner.name + "+intérieur", score: punchedScore };
    }
  }
