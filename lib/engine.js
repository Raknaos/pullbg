import { classifyImage } from "./classify.js?v=21";
import {
  bitmapFromSource,
  imageDataFromBitmap,
  floodBlack,
...
export async function refineCut(file, draft, sourceImage) {
  ...
  try {
    ...
    const fallback = bestOf([draft, ...fallbackCandidates(draft.image, draft.guess)]);
    return { image: fallback.image, guess: draft.guess, pipeline: fallback.name, score: fallback.score, needsRefine: false };
  } catch {
