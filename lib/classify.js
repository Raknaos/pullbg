...
function interiorMass(lum, w, h, pred, minFrac) {
  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      const pane = bbox > 0 && area >= bbox * 0.72 && area >= minPane;
  ...
  return { largest: largest / n, total: total / n, count };
}
...
export function classifyImage(image) {
  ...
  const multiPane =
    (interiorBlack.count >= 2 && interiorBlack.total >= 0.1) ||
    (interiorMid.count >= 2 && interiorMid.total >= 0.08);
