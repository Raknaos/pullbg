  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      while (head < tail) {
        ...
        if (cx > 0) {
          const ni = cy * w + (cx - 1);
          if (pred(lum[ni]) && !seen[ni]) { seen[ni] = 1; qx[tail] = cx - 1; qy[tail] = cy; tail++; }
        }
        if (cx + 1 < w) {
          const ni = cy * w + (cx + 1);
          if (pred(lum[ni]) && !seen[ni]) { seen[ni] = 1; qx[tail] = cx + 1; qy[tail] = cy; tail++; }
        ...
        if (cy > 0) {
          const ni = (cy - 1) * w + cx;
          if (pred(lum[ni]) && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy - 1; tail++; }
        }
        if (cy + 1 < h) {
          const ni = (cy + 1) * w + cx;
          if (pred(lum[ni]) && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy + 1; tail++; }
  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      while (head < tail) {
        ...
        if (cx > 0) {
          const ni = cy * w + (cx - 1);
          if (lum[ni] < floor && !seen[ni]) { seen[ni] = 1; qx[tail] = cx - 1; qy[tail] = cy; tail++; }
        }
        if (cx + 1 < w) {
          const ni = cy * w + (cx + 1);
          if (lum[ni] < floor && !seen[ni]) { seen[ni] = 1; qx[tail] = cx + 1; qy[tail] = cy; tail++; }
        ...
        if (cy > 0) {
          const ni = (cy - 1) * w + cx;
          if (lum[ni] < floor && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy - 1; tail++; }
        }
        if (cy + 1 < h) {
          const ni = (cy + 1) * w + cx;
          if (lum[ni] < floor && !seen[ni]) { seen[ni] = 1; qx[tail] = cx; qy[tail] = cy + 1; tail++; }
