...
function backdropMatch(data, w, h, br, bg, bb, maxDist = 28) {
  const n = w * h;
  let all = 0;
  let center = 0;
  let centerN = 0;
  ...
  return {
    all: all / n,
    center: centerN ? center / centerN : 1,
...
function interiorMass(lum, w, h, pred, minFrac) {
  ...
  return { largest: largest / n, total: total / n, count, rectangular, panes };
}
...
export function classifyImage(image) {
  ...
  if ((closedBlack && border.darkFrac >= 0.15) || darkPanes || lightGrid || (multiPane && border.whiteFrac < 0.55 && !lightBackdrop)) {
  ...
  const back = backdropMatch(image.data, w, h, border.meanR, border.meanG, border.meanB);
  if (back.border >= 0.8 && back.all >= 0.55 && back.all <= 0.92 && back.center < 0.4) {
    return { kind: "fond uni", mode: "fond", interior: false, reason: "fond de studio uni" };
  }
