...
function ringHoleCount(lum, w, h, box) {
  ...
  let best = ellipseHoleCount(lum, w, h, cx, cy, rxMax, ryMax);
  if (Math.max(rxMax, ryMax) / Math.max(1, Math.min(rxMax, ryMax)) < 1.25) {
    for (const a of [1.35, 1.65, 2.0]) {
      const nWide = ellipseHoleCount(lum, w, h, cx, cy, rxMax, rxMax / a);
      const nTall = ellipseHoleCount(lum, w, h, cx, cy, ryMax / a, ryMax);
