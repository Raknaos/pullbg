...
export function classifyImage(image) {
  ...
  for (let p = 0, i = 0; p < tint.length; p++, i += 4) {
    ...
    tint[p] = dist >= 32 && lum[p] < paneWhite ? 255 : 0;
  }
  ...
  const darkPanes =
    ...
    (!lightBackdrop || interiorBlack.rectangular >= 4);
  const multiPane =
    darkPanes ||
  ...
  const lightGrid =
    similarPaneCount(image, gridPanes) >= 2 &&
    ...
    (!lightBackdrop || skyLikeCount(image, gridPanes) >= 2 || gridPanes.length >= 4);
